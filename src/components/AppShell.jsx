      ...
      <header className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1320px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-1" aria-label="主导航">
            <Button asChild variant="ghost" size="sm" className="hidden rounded-full sm:inline-flex"><a href="/scan/"><ScanLine /> 扫码</a></Button>
            <Button asChild variant="ghost" size="sm" className="rounded-full"><a href={session ? "/me/" : "/login/"}><UserRound /> {session ? "我的" : "登录"}</a></Button>
            {profile?.role === "admin" && <Button asChild variant="ghost" size="sm" className="hidden rounded-full sm:inline-flex"><a href="/admin/"><ShieldCheck /> 管理</a></Button>}
            <ThemeButton />
          </nav>
        </div>
      </header>
