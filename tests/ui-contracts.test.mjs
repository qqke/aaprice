...
test("keeps production UI interaction contracts", async () => {
  ...
  assert.match(compare, /查询报价/)
  assert.doesNotMatch(compare, /disabled=\{loading \|\| \(turnstileEnabled && !captchaToken\)\}/)
  assert.match(compare, /登录后将自动继续查询价格，当前比价清单不会丢失。/)
  assert.match(compare, /authHref\("register"\)[\s\S]*?>注册账号<\/a>/)
  assert.match(compare, /authHref\("resetRequest"\)[\s\S]*?>忘记密码<\/a>/)
  assert.match(compare, /window\.location\.pathname/)
  assert.match(compare, /hasCatalogPrices \? <><div>[\s\S]*aria-label="最高预算"/)
  ...
  assert.match(product, /const mapUrl = online \? null : getMapUrl\(offer\)/)
  assert.match(product, /online \? "在线价" : "店头价"/)
  ...
  assert.match(product, /recordTelemetryEvent\("commercial_offer_seen", \{ offer_id: offer\.id, product_id: productId, source: "product" \}\)/)
  assert.match(product, /recordCommercialClick\(commercialOffer\.id, "product"\)/)
  assert.match(product, /购买可能为 LOWPRICE 带来收益/)
  assert.match(product, /const commercialSection = commercialOffer/)
  assert.doesNotMatch(product, /lg:sticky[\s\S]*?\{commercialOffer &&/)
  ...
  assert.match(account, /fetchProductsByIds\(relatedProductIds\)/)
  ...
  assert.match(admin, /adminFetchCommercialValidation\(\{ days: 30 \}\)/)
  assert.match(admin, /出口点击率目标 ≥ 8%/)
  assert.match(admin, /setCommercialForm\(\{ \.\.\.blankCommercialOffer, product_id: candidate\.product_id \}\)/)
  ...
  assert.match(api, /rpc\("admin_fetch_commercial_candidates", \{ payload \}/)
  assert.match(api, /rpc\("admin_fetch_commercial_validation", \{ payload \}/)
  assert.match(api, /rpc\("admin_bulk_upsert_commercial_offers", \{ payload \}/)
...
test("price alert worker keeps secrets server-side and records every outcome", async () => {
  ...
  assert.match(syncWorkflow, /dry_run:[\s\S]*type: boolean[\s\S]*default: true/)
  assert.match(syncWorkflow, /github\.event_name == 'workflow_dispatch' && inputs\.dry_run \|\| false/)
  assert.match(syncWorkflow, /if \[ "\$DRY_RUN" = "true" \]; then[\s\S]*npm run sundrug:sync -- --dry-run[\s\S]*exit 0/)
...
test("publishes concise privacy and affiliate disclosures", async () => {
  ...
  assert.match(disclosure, /购买可能为 LOWPRICE 带来收益/)
