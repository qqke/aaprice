...
export async function adminFetchCommercialCandidates(payload = {}) {
  const session = await requireSession()
  try {
    const result = await rpc("admin_fetch_commercial_candidates", { payload }, session.access_token)
    return Array.isArray(result) ? result : []
  } catch (error) {
    if (/admin_fetch_commercial_candidates|PGRST202|schema cache/i.test(String(error?.message || error))) return []
    throw error
  }
}
...
export function mapProductRow(row) {
  ...
  const category = sourceCategory || (
    /化粧|コスメ|美容|乳液|化粧水|ファンデ|リップ|ネイル|日焼け|UV|クレンジング|洗顔|美容液|スキンケア/.test(productText) ? "护肤美妆"
      : /シャンプー|コンディショナー|ボディソープ|歯磨|ハミガキ/.test(productText) ? "日常护理"
        : /医薬|目薬|点眼|カプセル|軟膏|鎮痛|鼻炎|胃腸|かぜ|風邪/.test(productText) ? "医药品"
          : /サプリ|ビタミン|健康食品|プロテイン/.test(productText) ? "营养保健"
            : "其他"
  )
